...
static
void vtrace(long tags, int lvl, const char *fmt, va_list args) {
  char s[0x1000], *nl, *buf, *sep;
  unsigned i;
  int ret, tag;
  bool on;

  if (t_lvl < lvl || lvl >= T_LEVELS) {
    return;
  }
