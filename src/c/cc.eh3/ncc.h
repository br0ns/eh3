...
#define NDEFS		10240		/* number of macros */
...
#define O_F(op)		((op) & 0xf00000)	/* opcode flag */
#define O_T(op)		((op) >> 24)	/* instruction operand type */
#define O_MK(op, bt)	((op) | ((bt) << 24))

/* added by br0ns */
#define CC_LT (O_LT & ~O_CMP)
#define CC_GE (O_GE & ~O_CMP)
#define CC_EQ (O_EQ & ~O_CMP)
#define CC_NE (O_NE & ~O_CMP)
#define CC_LE (O_LE & ~O_CMP)
#define CC_GT (O_GT & ~O_CMP)
#define O_JLT (CC_LT | O_JCC)
#define O_JGE (CC_GE | O_JCC)
#define O_JEQ (CC_EQ | O_JCC)
#define O_JNE (CC_NE | O_JCC)
#define O_JLE (CC_LE | O_JCC)
#define O_JGT (CC_GT | O_JCC)
