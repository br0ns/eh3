...
#define die_op_not_supported(op)                                        \
  do {                                                                  \
    const char* cn = "<unknown>";                                       \
    const char* cf = "<unknown>";                                       \
    for (int i = 0; ocnames[i].name; i++) {                             \
      if ((ocnames[i].oc & O_C(op)) == ocnames[i].oc) {                 \
        cn = ocnames[i].name;                                           \
        break;                                                          \
      }                                                                 \
    }                                                                   \
    for (int i = 0; ofnames[i].name; i++) {                             \
      if (ofnames[i].of == O_F(op)) {                                   \
        cf = ofnames[i].name;                                           \
        break;                                                          \
      }                                                                 \
    }                                                                   \
    die("neatcc: instruction %s.%s %d%s (op = %#lx) not supported\n",   \
        cn, cf, T_SZ(O_T(op)), O_T(op) & T_MSIGN ? "s" : "u", (op));    \
  } while (0)

#ifdef DIE_OP_NOT_SUPPORTED
/* NB: The return value of `i_ins` is never used anywhere in neatcc, which is */
/*     why we can do this. */
#define i_ins(op, args...)                                      \
  if (i_ins((op), ## args)) {                                   \
    die_op_not_supported(op);                                   \
  }
#endif
...
#ifdef NEATCC_EH3
/* EH3 relocations have bit offsets. */
* 8
#endif
                );
...
void o_func_end(void)
{
    ...
    /* XXX: call ic_gencode twice to avoid align after prologue? */
